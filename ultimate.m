...
for it=1:ll
    daysread=xlsread('PLDNewVolume.csv',1,days(it));
    ...
    y=xlsread("PLDNewVolume.csv",1,data_sets(it));
    [s1, s2]=size(y);
    t=linspace(f_day,l_day,s1);%set 't' for the first plot

%Defining of timespans for ode45
    inj=xlsread("PLDNewVolume.csv",1,injections(it));
    [iii, bbb]=size(inj);
    nums=[];
    for i=1:iii
        if inj(i)==8
...
% parameter values put in struct variable 'd'
    d=[0.12,0.44,0.00031, 0.00056, 6.75, 66.68, 0.037, 0.809, 2];
% d.a=0.12;
% d.b=0.44;
% d.bk=0;
% d.n=0.00031;
...
%plot of the measured volumes
    figure(it)
%clf
    h1=plot(t,y,'ko','MarkerFaceColor','k');
    xlabel('Time (days)');
    ylabel('Volume)');
...
% err=predRecoveryErr_my(d,number_of_day,y,time_span,x1_0,x2_0,x3_0,x4_0);%calculating error between data and prediction
%err=predRecoveryErr_my(d)
%p0=[d.a, d.b, d.n];
    A=[];
    b=[];
    Aeq=[];
    beq=[];
%ub=[20,20,0.2,0.2,20,100,20,20];
%ub=[0.20,0.20,0.2,0.2,0.20,0.100,0.20,0.20];
